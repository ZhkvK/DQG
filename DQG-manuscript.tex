...
\begin{document}
	...
	\begin{figure*}
		...
		\caption{Dependence of the self-diffusion coefficient $D$ on the $q$-gap width for 3D systems: (\textbf{a}) Lennard-Jones fluids with densities $n = 1$---$2$, and (\textbf{b}) Yukawa fluids with screening parameters $\kappa = 1$---$3$. Solid lines represent linear fits for both cases. Gradient regions and dashed lines represent a non-diffusive area.}
		\label{DQG:Fig1}
	\end{figure*}
	...
	In contrast, this effect does not manifest in two-dimensional systems, where the given dependencies exhibit a smooth increase from the origin. However, a nonlinear dependence is observed for the 2D Yukawa system (Fig. \hyperref[DQG:Fig2]{2(b)}). To analyze this behavior, we employed a fit based on Eq.\eqref{DQG:Eq6}.
	\begin{equation}
		D \propto q_g^{-\lambda_D/\sqrt{n}}.
		\label{DQG:Eq6}
	\end{equation}
	
	We also investigated the dependency of the self-diffusion coefficient value in MP $D_{\text{MP}}$ on the slope $\dot{D} = \text{d}D/\text{d}q_g$ and observed an intriguing behavior, as shown in Figure~\ref{DQG:Fig3}. It appears that this universal relationship might be described by the phenomenological fit~\eqref{DQG:Eq7}:
	...
	where $C_{\text{MP}} = 9.2 \cdot 10^{-4}$. Using this fit we reconstructed initial data for 3D systems. Results are presented in \href{https://www.latextemplates.com}{supplementary material}.
