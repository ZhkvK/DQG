...
%	\onecolumngrid 
	...
	The dependencies of self-diffusion coefficient on $q$-gap width for 3D LJ and Yukawa systems as well as for liquid \ce{Al} and \ce{Fe} are presented on Figure \ref{DQGSM:Fig4}. Solid lines represent direct linear approximations, consistent with dependencies obtained using phenomenological form from main article. Using these approximations we calculated the slope $\dot{D} = \text{d}D/\text{d}q_g$. Obtained $q$-gap values near the melting point (MP) -- $q_{g, \text{MP}}$ are presented in Table \ref{DQGSM:Table2} along with the slopes of $D(q_g)$ dependencies for each considered system.
	
	Moreover, we compared the results for the Lennard–Jones and Yukawa fluids with those obtained for extended systems in the same states. The simulation box was enlarged such that $L_{\mathrm{ext}} = 1.5 L_{\mathrm{orig}}$. The resulting dependencies closely match those of the original systems, as illustrated in Fig.~\ref{DQGSM:Fig5}. Data for the original systems are shown in red, and those for the extended systems in blue, along with their respective linear fits.
