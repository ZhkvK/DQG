...
\begin{document}
	...
	The dependencies of the self-diffusion coefficient, calculated using particle MSD given by Eq. \eqref{DQG:Eq5}, on the $q$-gap width for the considered systems are shown in Figure \ref{DQG:Fig1} for both three- and two-dimensional cases.
	
	The plots display values corresponding to the $q$-gap along with their approximations. A clear linear relationship between the coefficient $D$ and the $q$-gap width is observed for three-dimensional cases and, notably, it does not pass through the origin, which corresponds to a crystalline state with zero values of $D$ and $q$-gap. Furthermore, the results obtained for the melting point in 3D systems appear to indicate the formation of a non-diffusive region represented by a gray area on the plots. The calculated values of the $q$-gap at the melting point, denoted as $q_{0}$, are presented in the table provided in the \href{https://www.latextemplates.com}{supplementary material}.
	
	\begin{figure*}
		\centering
		\includegraphics[width=\linewidth]{Figs/Fig1}
		\caption{Dependence of the self-diffusion coefficient $D$ on the $q$-gap width for (\textbf{a}),(\textbf{b}) Lennard-Jones and (\textbf{c}),(\textbf{d}) Yukawa fluids in 3D and 2D cases. For Lennard-Jones: 3D systems with densities $n = 1$--$2$ and 2D systems with $n = 0.8$--$0.9$, solid lines are linear fits. For Yukawa: 3D and 2D systems with screening parameters $\kappa = 1$---$3$ and $\kappa = 1$---$2$ respectively, solid lines indicate linear approximations in 3D case and fit by Eq.\eqref{DQG:Eq6} in 2D.}
		\label{DQG:Fig1}
	\end{figure*}
	...
	In contrast, this effect does not manifest in two-dimensional systems, where the given dependencies exhibit a smooth increase from the origin. However, a nonlinear dependence is observed for the 2D Yukawa system (Fig. \hyperref[DQG:Fig1]{1(d)}). To analyze this behavior, we employed a fit based on Eq.\eqref{DQG:Eq6}.
	\begin{equation}
		D \propto q_g^{-\lambda_D/\sqrt{n}}.
		\label{DQG:Eq6}
	\end{equation}
	
	We also investigated the dependency of the self-diffusion coefficient on the slope $\text{d}D/\text{d}q_g$ and observed an intriguing behavior, as shown in Figure~\ref{DQG:Fig2}. It appears that this universal relationship might be described by the theoretical fit~\eqref{DQG:Eq7}:
	
	\begin{equation}
		D \approx 7.4\exp\left[30\left( \frac{\text{d}D}{\text{d}q_g}\right)\right]\cdot10^{-4}
		\label{DQG:Eq7}
	\end{equation}
